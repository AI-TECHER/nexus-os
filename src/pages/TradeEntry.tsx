...
const PAYOUTS = [10, 60, 65, 70, 75, 80, 82, 83, 84, 85, 87, 90, 92, 93, 94, 95];
...
export default function TradeEntry({ trades, setTrades, settings, recovery, onWarnings, getAvailableBalance }: TradeEntryProps) {
  ...
  // Track if user manually changed the amount
  ...
  // ── Auto-calculate trade amount based on risk management ──
  const calculateAutoAmount = (): number => {
    const balance = getAvailableBalance();
    // Use max risk per trade from settings (default 2%)
    const riskPercent = settings.maxRiskPerTrade || 2;
    const calculatedAmount = (balance * riskPercent) / 100;
    // Round to nearest 10 and ensure minimum 100
    const roundedAmount = Math.max(100, Math.round(calculatedAmount / 10) * 10);
    return roundedAmount;
  };

  // Auto-fill amount when Fixed Time Trading is selected and not manually changed
  useEffect(() => {
    if (isFT && !isAmountManuallyChanged) {
      const autoAmount = calculateAutoAmount();
      setForm(prev => ({ ...prev, amount: String(autoAmount) }));
    }
  }, [isFT, isAmountManuallyChanged, settings.maxRiskPerTrade, getAvailableBalance]);

  // Reset manual change flag when market type changes
  useEffect(() => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Ensure amount is at least 100
    const amount = parseFloat(form.amount) || 0;
    if (amount < 100) {
      alert(`Minimum trade amount is ${formatCurrency(100, CS)}`);
      return;
    }
    ...
    // Reset form but keep amount auto-filled for next trade
    setForm(prev => ({
      ...defaultForm,
      marketType: prev.marketType,
      date: prev.date,
      time: prev.time,
      // Keep amount if it was manually set, otherwise it will auto-fill
      amount: isAmountManuallyChanged ? prev.amount : String(calculateAutoAmount()),
    }));
    setIsAmountManuallyChanged(false);
  };
  ...
  const handleEdit = (trade: Trade) => {
    ...
    // When editing, mark amount as manually changed so it doesn't auto-fill
  ...
  // Calculate current trade P&L for the sticky bar
  const currentPnL = calcPnL();
  const currentPayout = isFT ? form.payoutPercent : null;
  const currentExpiry = isFT ? getResolvedDuration() : null;

  // Auto-calculate amount for display
  const autoAmount = calculateAutoAmount();

  // Handle amount change - mark as manually changed when user types
  const handleAmountChange = (value: string) => {
    setForm(prev => ({ ...prev, amount: value }));
    setIsAmountManuallyChanged(true);
  };

  // ── Handle Amount Input Blur - Ensure minimum amount ──
  const handleAmountBlur = () => {
    const currentAmount = parseFloat(form.amount) || 0;
    if (currentAmount < 100 && currentAmount > 0) {
      setForm(prev => ({ ...prev, amount: '100' }));
    }
  };

  // ── Handle Auto Risk Button Click ──
  const handleAutoRiskClick = () => {
    const autoAmount = calculateAutoAmount();
    setForm(prev => ({ ...prev, amount: String(autoAmount) }));
    setIsAmountManuallyChanged(false);
  };
      ...
      {showForm && (
        <motion.form initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} onSubmit={handleSubmit} className="glass-card p-5 space-y-4">
          ...
          <motion.div 
            ...
            className="p-3 rounded-lg bg-[rgba(0,212,255,0.05)] border border-[rgba(0,212,255,0.15)] flex items-center justify-between flex-wrap gap-3 sticky top-0 z-10"
          >
            <div className="flex items-center gap-4 flex-wrap">
              {isFT && (
                <>
                  <div>
                    <span className="text-[9px] text-gray-500">Trade P&L</span>
                    <div className={`text-sm font-bold ${(parseFloat(currentPnL) || 0) >= 0 ? 'text-[#00ff88]' : 'text-[#ff3366]'}`}>
                      {currentPnL === '—' ? '—' : formatCurrency(parseFloat(currentPnL) || 0, CS)}
                    </div>
                  </div>
                  <div>
                    <span className="text-[9px] text-gray-500">Payout</span>
                    <div className="text-sm font-bold text-[#00d4ff]">{currentPayout}%</div>
                  </div>
                  <div>
                    <span className="text-[9px] text-gray-500">Expiry</span>
                    <div className="text-sm font-bold text-[#a855f7]">{currentExpiry || '—'}</div>
                  </div>
                </>
              )}
              {!isFT && (
                <>
                  <div>
                    <span className="text-[9px] text-gray-500">Est. P/L</span>
                    <div className={`text-sm font-bold ${(parseFloat(currentPnL) || 0) >= 0 ? 'text-[#00ff88]' : 'text-[#ff3366]'}`}>
                      {currentPnL === '—' ? '—' : formatCurrency(parseFloat(currentPnL) || 0, CS)}
                    </div>
                  </div>
                  <div>
                    <span className="text-[9px] text-gray-500">R:R Ratio</span>
                    <div className="text-sm font-bold text-[#00d4ff]">{calcRR()}</div>
                  </div>
                </>
              )}
            </div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-bold text-[#a855f7]">{formattedBalance}</span>
              {isFT && (
                <button 
                  type="button" 
                  onClick={handleAutoRiskClick}
                  className="px-2 py-1 rounded text-[10px] bg-[rgba(0,212,255,0.1)] text-[#00d4ff] hover:bg-[rgba(0,212,255,0.2)] transition-all"
                >
                  Auto Risk
                </button>
              )}
          ...
          {/* Row 2: Prices — HIDDEN for FT */}
          {!isFT && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div>
                <label className="text-[10px] text-gray-500 uppercase tracking-wider mb-1 block">Entry Price *</label>
                <input className="cyber-input" type="number" step="any" placeholder="0.00" value={form.entryPrice} onChange={e => setForm({ ...form, entryPrice: e.target.value })} required />
              </div>
              <div>
                <label className="text-[10px] text-gray-500 uppercase tracking-wider mb-1 block">Exit Price *</label>
                <input className="cyber-input" type="number" step="any" placeholder="0.00" value={form.exitPrice} onChange={e => setForm({ ...form, exitPrice: e.target.value })} required />
              </div>
              <div>
                <label className="text-[10px] text-gray-500 uppercase tracking-wider mb-1 block">Stop Loss</label>
                <input className="cyber-input" type="number" step="any" placeholder="0.00" value={form.stopLoss} onChange={e => setForm({ ...form, stopLoss: e.target.value })} />
              </div>
              <div>
                <label className="text-[10px] text-gray-500 uppercase tracking-wider mb-1 block">Take Profit</label>
                <input className="cyber-input" type="number" step="any" placeholder="0.00" value={form.takeProfit} onChange={e => setForm({ ...form, takeProfit: e.target.value })} />
              </div>
            </div>
          )}

          {/* Row 3: Amount / Payout / Result / Date / Time */}
          <div className={`grid gap-3 ${isFT ? 'grid-cols-2 md:grid-cols-5' : 'grid-cols-2 md:grid-cols-5'}`}>
            <div>
              <label className="text-[10px] text-gray-500 uppercase tracking-wider mb-1 block">
                {isFT ? `Trade Amount * (min ${formatCurrency(100, CS)})` : 'Amount/Lots'}
              </label>
              <input 
                ...
                step="any" 
                min="100"
                placeholder={isFT ? `Min ${formatCurrency(100, CS)}` : 'Lots'} 
                value={form.amount} 
              ...
              />
              {isFT && (
                <div className="text-[8px] text-gray-600 mt-0.5 flex items-center gap-2">
                  <span>💡 Auto: {formatCurrency(autoAmount, CS)}</span>
                  {isAmountManuallyChanged && (
                    <span className="text-[#ffdd00]">✏️ Manual override</span>
                  )}
                </div>
              )}
            </div>
